...
for x in range(number_of_inputs):
    ...
    bytes_transaction_id = transaction_id.to_bytes(4,byteorder='big', signed=True)
    output_bytes.extend(bytes_transaction_id)

    #print(transaction_id)
    index = int(input("enter index"))
    bytes_index = index.to_bytes(4,byteorder='big', signed=True)
    output_bytes.extend(bytes_index)

    signature = int(input("enter signature"),16)
    bytes_signature = signature.to_bytes(4,byteorder='big', signed=True)
    output_bytes.extend(bytes_signature)

number_of_outputs = int(input("Please enter number of outputs"))
bytes_number_of_outputs = number_of_outputs.to_bytes(4,byteorder='big', signed=True)
...
for x in range(number_of_outputs):
    ...
    bytes_number_of_coins = number_of_coins.to_bytes(8,byteorder='big', signed=True)
